...
function Product() {
  ...
  if (current) {
    return (
      <>
        <Info title={` أشكال وصفات المنتج ${current?.name} `} back={true} />
        ...
        <div className="grid grid-cols-3 gap-x-16 py-4">
          ...
          <div className=" flex flex-col text-sm gap-y-5">
            <span> رمز المنتج : {current?.sku ?? 0} </span>
            <span>
              سعر البيع : {current?.base_price ?? 0}$ = :{current?.price ?? 0}{" "}
              ل.س{" "}
            </span>
            <span> سعر الصرف الان : {current?.rate ?? 0} ل.س </span>
