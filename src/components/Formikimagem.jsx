...
export default function Formikimagem({
  ...
  */
  ...
  const previews = useMemo(() => {
    return files.map((item) => ({
      url: item.preview || item.path_url,
      isFile: !!item.file,
    }));
  }, [files]);
  ...
  */
  ...
  const handleChange = (e) => {
    ...
    const existingNames = files.map((item) => item.file?.name || item.path_url);
    ...
    const mergedFiles = [...files, ...newFiles].slice(0, maxFiles);

    helpers.setValue(mergedFiles);
